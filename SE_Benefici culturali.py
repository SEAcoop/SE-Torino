...
class ExampleProcessingAlgorithm(QgsProcessingAlgorithm):
    ...
    def displayName(self):
        ...
        """
        return self.tr('SE Benefici culturali')
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        score_lucode_present = np.zeros((rows, cols))
        acc_score_present = np.zeros((rows, cols))
        n_valid_lucode_present = 0
        # Summing scores for each distinct lucode
        for lucode in np.unique(arr_present):
            if lucode in score_lucode.keys():
                n_lucode = np.sum(arr_present == lucode)
                score_lucode_present[np.where(arr_present == lucode)] = score_lucode[lucode]
            ...
            if lucode in score_acc.keys():
                acc_score_present[np.where(arr_present == lucode)] = score_acc[lucode]
        PR_present = (score_lucode_present + natural_aspects_norm_pres + urban_green_norm_pres) / 3
        acc_fr_present = (acc_score_present / 2.3 + fruibility_norm_pres) / 2
        ROS_present_array = PR_present * 0.3 + (acc_fr_present * 0.7)
        # ROS_present_array = (ROS_present_array * ROS_present)
        ROS_present = np.sum(np.unique(ROS_present_array))
        value_tot_present = 0
        for lucode in np.unique(arr_present):
            if lucode in value_lucode.keys():
                value_tot_present += np.sum(area_pixel * ROS_present_array[np.where(arr_present == lucode)] * value_lucode[lucode])
        ...
        score_lucode_future = np.zeros((rows, cols))
        acc_score_future = np.zeros((rows, cols))
        n_valid_lucode_future = 0
        # Summing scores for each distinct lucode
        for lucode in np.unique(arr_future):
            if lucode in score_lucode.keys():
                n_lucode = np.sum(arr_future == lucode)
                score_lucode_future[np.where(arr_future == lucode)] = score_lucode[lucode]
            ...
            if lucode in score_acc.keys():
                acc_score_future[np.where(arr_future == lucode)] = score_acc[lucode]
        PR_future = (score_lucode_future + natural_aspects_norm_pres + urban_green_norm_pres) / 3
        acc_fr_future = (acc_score_future / 2.3 + fruibility_norm_pres) / 2
        ROS_future_array = PR_future * 0.3 + (acc_fr_future * 0.7)
        # ROS_future_array = (ROS_future_array * ROS_future)
        ROS_future = np.sum(np.unique(ROS_future_array))
        value_tot_future = 0
        for lucode in np.unique(arr_future):
            if lucode in value_lucode.keys():
                value_tot_future += np.sum(area_pixel * ROS_future_array[np.where(arr_future == lucode)] * value_lucode[lucode])
        # Years
        ...
        if self.parameterAsBool(parameters, self.BELP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Belvedere (punto panoramico): %s \n" % binary)
        if self.parameterAsBool(parameters, self.FIU1P, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Corso acqua primario nell area o entro 200m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.NATP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Area Natura 2000 o area protetta: %s \n" % binary)
        if self.parameterAsBool(parameters, self.FIU2P, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Corso acqua secondario nell area o entro 200m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.ALBP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Albero Monumentale: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PA1P, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Parco urbano >2 ha: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PA2P, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Parco urbano tra 0,5 e 2 ha: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PA3P, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Parco urbano < 0,5 ha: %s \n" % binary)
        if self.parameterAsBool(parameters, self.GIAP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di un Giardino storico: %s \n" % binary)
        if self.parameterAsBool(parameters, self.GIOP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Area giochi in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PIAP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Piastra sportiva attrezzata in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.CANP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Area cani in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.CHIP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Chiosco in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.SERP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Servizi igienici in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.FONP, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Fontana in un buffer di 100m: %s \n" % binary)

        f.write(
            "Valore ROS - Stato attuale : : %f \n" % (
                ROS_present))
        f.write(
            "Valore medio dei benefici_culturali per unità di superficie - Stato attuale (€/ha): : %f \n" % (
        ...
        if self.parameterAsBool(parameters, self.BELP, context):
            ...
            binary = 'no'
        f.write("Presenza di Belvedere (punto panoramico): %s \n" % binary)
        if self.parameterAsBool(parameters, self.FIU1F, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Corso acqua primario nell area o entro 200m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.NATF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Area Natura 2000 o area protetta: %s \n" % binary)
        if self.parameterAsBool(parameters, self.FIU2F, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Corso acqua secondario nell area o entro 200m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.ALBF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Albero Monumentale: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PA1F, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Parco urbano >2 ha: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PA2F, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Parco urbano tra 0,5 e 2 ha: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PA3F, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Parco urbano < 0,5 ha: %s \n" % binary)
        if self.parameterAsBool(parameters, self.GIAF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di un Giardino storico: %s \n" % binary)
        if self.parameterAsBool(parameters, self.GIOF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Area giochi in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.PIAF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Piastra sportiva attrezzata in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.CANF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Area cani in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.CHIF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Chiosco in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.SERF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Servizi igienici in un buffer di 100m: %s \n" % binary)
        if self.parameterAsBool(parameters, self.FONF, context):
            binary = 'sì'
        else:
            binary = 'no'
        f.write("Presenza di Fontana in un buffer di 100m: %s \n" % binary)
        f.write(
            "Valore ROS - Stato progetto : : %f \n" % (
                ROS_future))
        f.write(
            "Valore medio dei benefici_culturali per unità di superficie - Stato di progetto (€/ha): %f \n" % (
        ...
        f.write("Differenza dei benefici_culturali per unità di superficie (€/ha): %f \n" % (
